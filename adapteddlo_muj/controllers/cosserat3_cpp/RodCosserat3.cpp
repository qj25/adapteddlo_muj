...
void RodCosserat3::updateStiffnessFromMaterial()
{
...
void RodCosserat3::update_XVecs(const double* node_pos)
{
    for (int i = 0; i < nv + 2; i++) {
...
void RodCosserat3::updateE2K()
{
    ...
    nodes[nv + 1].k = 0.0;
    for (int i = 1; i < nv + 1; i++) {
        nodes[i].phi_i = Cosserat3Utils::calculateAngleBetween(edges[i - 1].e, edges[i].e);
        nodes[i].k = 2. * tan(nodes[i].phi_i / 2.);
    }
}

void RodCosserat3::updateE2Kb()
{
    nodes[0].kb << 0., 0., 0.;
    nodes[nv + 1].kb << 0., 0., 0.;
    for (int i = 1; i < nv + 1; i++) {
...
bool RodCosserat3::transfBF(const Eigen::Matrix3d& bf_0)
{
    bool bf_align = true;
    ...
    for (int i = 1; i < nv + 1; i++) {
        ...
        if (nodes[i].kb.norm() == 0) {
            ...
            edges[i].bf.row(1) = Cosserat3Utils::rotateVector3(
                edges[i - 1].bf.row(1),
...
Eigen::Vector3d RodCosserat3::materialCurvatureAtNode(int i) const
{
...
void RodCosserat3::updateMaterialCurvature()
{
    for (int i = 0; i < nv + 2; i++) {
...
void RodCosserat3::captureRestCurvature()
{
    for (int i = 0; i < nv + 2; i++) {
...
Eigen::Vector3d RodCosserat3::kbEffectiveAtNode(int i) const
{
    const Eigen::Matrix3d& R = edges[i].bf;
...
void RodCosserat3::updateThetaN(double theta_n)
{
...
double RodCosserat3::updateTheta(const double theta_n)
{
...
void RodCosserat3::resetTheta(const double theta_n, const double overall_rot)
{
    p_thetan = theta_n;
    edges[nv].theta = overall_rot;
}

void RodCosserat3::changeAlphaBeta(const double a_bar, const double b_bar)
{
    alpha_bar = a_bar;
    beta_bar = b_bar;
    updateStiffnessFromMaterial();
...
void RodCosserat3::calculateKirchhoffForces_sub(const int start_i, const int end_i)
{
    for (int i = start_i; i < end_i; i++) {
        nodes[i].nabkb[0] = (
            (
                2 * Cosserat3Utils::createSkewSym(edges[i].e)
        ...
        nodes[i].nabkb[2] = (
            (
                2 * Cosserat3Utils::createSkewSym(edges[i - 1].e)
...
void RodCosserat3::calculateCenterlineF2(int dim_nf, double* node_force)
{
    (void)dim_nf;
    for (int i = 0; i < (nv + 2); i++) {
        nodes[i].force << 0., 0., 0.;
...
void RodCosserat3::calculateCenterlineTorq(
...
double RodCosserat3::calculateTwistingEnergy()
{
    const double dtheta = (edges[nv].theta - edges[0].theta) - theta_star_total_;
    return 0.5 * k_twist_ * dtheta * dtheta / bigL_bar;
}

double RodCosserat3::calculateEnergy()
{
    return calculateBendingEnergy() + calculateTwistingEnergy();
}

void RodCosserat3::initQe_o2m_loc(int dim_qo2m, double* q_o2m)
{
    (void)dim_qo2m;
...
void RodCosserat3::calculateOf2Mf(
