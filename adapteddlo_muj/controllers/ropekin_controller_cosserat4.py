"""
Centerline elastic rope controller with fullDyn twist (per-link stored twist).

Uses RodCosserat4 C++ backend; torques applied via qfrc_passive (adapt path).
"""
...
import adapteddlo_muj.controllers.cosserat4_cpp.RodCosserat4 as RodCosserat4
...
class DLORopeCosserat4:
    ...
    def _scaled_alpha_beta(self):
        return scale_material(
            self.alpha_bar, self.beta_bar, material_scale_for("cosserat4")
        )

    def _init_cosserat4_cpp(self):
        self._update_xvecs()
        self._init_resetbody_vars()
        self._x2e()
        self._init_bf()
        alpha_cpp, beta_cpp = self._scaled_alpha_beta()
        self.dlo_math = RodCosserat4.RodCosserat4(
            self.x.flatten(),
    ...
    def _update_cosserat4_cpp(self):
        self._update_xvecs()
        ...
        self.dlo_math.updateThetasFullDyn(body_quats_flat)
        self.overall_rot = self.dlo_math.overall_rot
        return bf_align
    ...
    def _init_o2m(self):
        ...
        body_quats_flat = self.data.xquat[self.vec_bodyid[:]].flatten()
        self.dlo_math.initO2MLocAll(body_quats_flat)
