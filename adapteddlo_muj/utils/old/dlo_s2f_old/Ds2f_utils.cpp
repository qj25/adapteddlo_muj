...
std::pair<Eigen::Vector3d, double> Ds2fUtils::computeLeastSquares(
    ...
    Eigen::MatrixXd Vh = svd.matrixV().transpose();
    
    // Create pseudoinverse of Σ
    Eigen::MatrixXd S_pinv = Eigen::MatrixXd::Zero(A.cols(), A.rows());
    for (int i = 0; i < std::min(A.rows(), A.cols()); i++) {
        if (S(i) > 1e-10) {  // Only use non-zero singular values
            S_pinv(i, i) = 1.0 / S(i);
        }
    }
    
    // Compute solution
    Eigen::Vector3d F = Vh.transpose() * S_pinv * U.transpose() * c;
    
    // Compute residuals
    Eigen::VectorXd residual = A * F - c;
    double residual_norm = residual.norm();
    
    return std::make_pair(F, residual_norm);
}
...
const Eigen::VectorXd Ds2fUtils::solveUTXBig(const Eigen::MatrixXd &A, const Eigen::VectorXd &C)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 1; j < n; ++j) {
            ...
            // Csingle -= A.block(i,j*3,1,3).dot(x_sol.segment(j*3,3)));
        }
        x_sol(i) = Csingle;
    }
    // std::cout << 'b' << std::endl;
