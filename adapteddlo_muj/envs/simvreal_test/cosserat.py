...
def _load_case(wire_color: str, pos_id: int):
    picklename = os.path.join(
        ...
        "simdata",
        "plugin",
        f"simdata_{wire_color}{pos_id}_cosserat.pickle",
    )
    if os.path.exists(picklename):
        return load_sim_pickle(wire_color=wire_color, pos_id=pos_id, model_name="cosserat")
    return load_sim_pickle(wire_color=wire_color, pos_id=pos_id, model_name="adapt")


def get_model_spec():
    return {"name": "cosserat", "load_case": _load_case}
