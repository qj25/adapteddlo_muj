...
from adapteddlo_muj.utils.mjc_utils import get_sensor_id, get_sensor_force2


class TestFQSEnv(gym.Env, utils.EzPickle):
    def __init__(
        self,
        do_render=False,
    ...
    def _init_ids(self):
        ...
        self.box_mid_relpose[3:] = T.quat_multiply(
            ...
            self.data.xquat[self.eef_body3_id]
        )
        mujoco.mju_rotVecQuat(
            self.box_mid_relpose[:3],
            (
                self.data.xpos[self.eef_body3_id]
    ...
    def _midboxpos_follow(self):
        mujoco.mj_fwdPosition(self.model, self.data)
        self.midbox_worldpose = np.zeros(7)
        self.midbox_worldpose[3:] = T.quat_multiply(
            self.box_mid_relpose[3:],
    ...
    def _get_xmlstr(self):
        ...
        eef_body3 = XMLWrapper(eef_body3_path)
    ...
    def set_curve(self, om):
        ...
        self._boxpos_follow()
        self._midboxpos_follow()
        mjc2.update_weld_relpose(self.model, self.data, "weld_end")
        mjc2.update_weld_relpose(self.model, self.data, "weld_mid")
    ...
    def rot_x_rads2_sdir(self, x_rads):
        x_rads /= 2.0   # rotate from both ends in the SAME direction
        body_id = mjc2.obj_name2id(self.model,"body","eef_body")
    ...
    def ropeend_rot2_sdir(self, body_id, rot_a=np.pi/180, rot_axis=0):
        rot_arr = np.zeros(3)
        rot_arr[rot_axis] = rot_a
        rot_quat = T.axisangle2quat(-rot_arr)
        rot_quat2 = T.axisangle2quat(-rot_arr)   # rotate in the same global direction
        new_quat = T.quat_multiply(rot_quat, self.model.body_quat[self.ropeend_body_id])
        new_quat2 = T.quat_multiply(rot_quat2, self.model.body_quat[body_id])
        self.model.body_quat[self.ropeend_body_id] = new_quat
        self.model.body_quat[body_id] = new_quat2
