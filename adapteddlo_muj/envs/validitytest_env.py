...
class TestPluginEnv(gym.Env, utils.EzPickle):
    def __init__(
        self,
        do_render=False,
    ...
    def lhb_var_compute(self):
        ...
        max_devi2 = max_devi
        # max_devi2 = 0.919
        m_const = self.overall_rot/(
            self.r_len
            * ((self.r_pieces-1)/self.r_pieces)
        )
        s_ss = (
    ...
    def start_lhbtest(self, new_start):
        ...
        lhb_picklename = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "data/lhb/" + self.data_foldername + lhb_picklename
        )
        if new_start:
            self.lhb_init()
            self.init_pickle = self.get_state()
        ...
        pickledata_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "data/lhb/" + self.data_foldername + pickledata_path
        )
    ...
    def start_circletest(self, new_start):
        ...
        circtest_picklename = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "data/mbi/" + self.data_foldername + circtest_picklename
        )
        if new_start:
